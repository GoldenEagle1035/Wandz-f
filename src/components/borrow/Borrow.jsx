                  ...
                  <tbody>
                    {collections.map((collection, index) => (
                      ...
                      <tr className="py-10 border-b-[1px] border-[#a9a9a9d8] max-sm:px-4">
                        ...
                        <td className=" max-sm:px-4 pl-4 max-sm:text-[11px] text-nowrap">
                          <span className="text-lg mr-1">θ {formatUnits(loans.loans.filter((loan) => loan.nftAddress.toLowerCase() == collection.address.toLowerCase() && loan.duration == collection.duration && loan.amount != 0 && !loan.accepted && !loan.paid && !loan.liquidated).reduce(((total, loan) => total + loan.amount), 0), 18)}</span>
                          <br />
                        ...
                        <td className="max-sm:text-[11px] pl-4 max-sm:px-4 text-nowrap">
                          <span className="text-lg mr-1">θ {formatUnits(loans.loans.filter((loan) => loan.nftAddress.toLowerCase() == collection.address.toLowerCase() && loan.duration == collection.duration && loan.amount != 0 && !loan.accepted && !loan.paid && !loan.liquidated).length != 0 ? loans.loans.filter((loan) => loan.nftAddress.toLowerCase() == collection.address.toLowerCase() && loan.duration == collection.duration && loan.amount != 0 && !loan.accepted && !loan.paid && !loan.liquidated).sort((a, b) => a.amount - b.amount).at(0).amount : 0, 18)}</span> <br />
                          <span className="text-[9px] max-sm:text-[8px] text-[#B5B5B5]">
                            θ {formatUnits(loans.loans.filter((loan) => loan.nftAddress.toLowerCase() == collection.address.toLowerCase() && loan.duration == collection.duration && loan.amount != 0 && !loan.accepted && !loan.paid && !loan.liquidated).length != 0 ? loans.loans.filter((loan) => loan.nftAddress.toLowerCase() == collection.address.toLowerCase() && loan.duration == collection.duration && loan.amount != 0 && !loan.accepted && !loan.paid && !loan.liquidated).at(loans.loans.filter((loan) => loan.nftAddress.toLowerCase() == collection.address.toLowerCase() && loan.duration == collection.duration && loan.amount != 0 && !loan.accepted && !loan.paid && !loan.liquidated).length - 1).amount : 0, 18)} last loan taken
                          </span>{" "}
                        </td>
                    ...
                    {/* {loans.loans.map((loan, index) => (
                      ...
                      <tr className="py-10 border-b-[1px] border-[#a9a9a9d8] max-sm:px-4">
                        ...
                        <td className=" max-sm:px-4 pl-4 max-sm:text-[11px]">
                          <span className="text-lg mr-1">θ</span>{formatUnits(loans.loans.filter((t_item) => t_item.nftAddress.toLowerCase() == loan.nftAddress.toLowerCase() && t_item.duration == loan.duration && t_item.amount != 0 && !t_item.accepted && !t_item.paid && !t_item.liquidated).reduce(((total, tt_item) => total + tt_item.amount), 0), 18)}
                          <br />
                          <span className="text-[9px]  max-sm:text-[8px] text-[#B5B5B5]">
                        ...
                        <td className="max-sm:text-[11px] pl-4 max-sm:px-4">
                          <span className="text-lg mr-1">θ</span>{formatUnits(loans.loans.filter((t_item) => t_item.nftAddress.toLowerCase() == loan.nftAddress.toLowerCase() && t_item.duration == loan.duration && t_item.amount != 0 && !t_item.accepted && !t_item.paid && !t_item.liquidated).sort((a, b) => a.amount - b.amount).at(0).amount, 18)} <br />
                          <span className="text-[9px] max-sm:text-[8px] text-[#B5B5B5]">
                            θ {formatUnits(loans.loans.filter((t_item) => t_item.nftAddress.toLowerCase() == loan.nftAddress.toLowerCase() && t_item.duration == loan.duration && t_item.amount != 0 && !t_item.accepted && !t_item.paid && !t_item.liquidated).at(loans.loans.filter((t_item) => t_item.nftAddress.toLowerCase() == loan.nftAddress.toLowerCase() && t_item.duration == loan.duration && t_item.amount != 0 && !t_item.accepted && !t_item.paid && !t_item.liquidated).length - 1).amount, 18)} last loan taken
                          </span>{" "}
                        </td>
                  ...
                  {collections.map((collection, index) => (
                    ...
                    <div className="py-4 border-b-[1px] border-[#a9a9a9d8] flex flex-col gap-[5px]">
                      ...
                      <div className="flex gap-[5px] text-white text-[11px]">
                        ...
                        <span className="w-1/4 text-center">θ {formatUnits(loans.loans.filter((loan) => loan.nftAddress.toLowerCase() == collection.address.toLowerCase() && loan.duration == collection.duration && loan.amount != 0 && !loan.accepted && !loan.paid && !loan.liquidated).length != 0 ? loans.loans.filter((loan) => loan.nftAddress.toLowerCase() == collection.address.toLowerCase() && loan.duration == collection.duration && loan.amount != 0 && !loan.accepted && !loan.paid && !loan.liquidated).sort((a, b) => a.amount - b.amount).at(0).amount : 0, 18)}</span>
          ...
          <div className="min-w-[300px] max-w-[400px] bg-[#D9D9D930] backdrop-blur-sm flex gap-[20px] flex-col rounded-[10px] p-[10px]" >
            ...
            <div className="w-full flex gap-[20px] justify-between">
              <div className="flex flex-col items-center">
                ...
                <span className="text-[14px] font-[400] text-[#DBFF00]">θ {formatUnits(loans.loans.filter((loan) => loan.nftAddress.toLowerCase() == collections[selectedCollection].address.toLowerCase() && loan.duration == collections[selectedCollection].duration && loan.amount != 0 && !loan.accepted && !loan.paid && !loan.liquidated).reduce(((total, loan) => total + loan.amount), 0), 18)}</span>
              </div>
              <div className="flex flex-col items-center">
            ...
            <div className="max-h-[50vh] overflow-y-auto w-full flex flex-col gap-[10px] px-[20px]">
              {loans.loans.map((loan, index) => (
                ...
                <div onClick={(e) => onAcceptOffer(index)} className="flex gap-[10px] items-center cursor-pointer">
                  ...
                  <span className="w-1/2 p-4 pl-4 max-sm:text-[11px] text-[14px] text-center text-white max-sm:px-4">θ {formatUnits(loan.amount, 18)}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="min-w-[300px] max-w-[400px] bg-[#D9D9D930] backdrop-blur-sm flex gap-[20px] flex-col rounded-[10px] p-[20px]" >
            <div className="w-full flex gap-[20px] justify-between items-center">
              ...
              <div className="flex flex-col items-center bg-[#D9D9D930] border border-[#DBFF0030] rounded-[10px] px-[20px] py-[5px]">
                ...
                <span className="text-[14px] font-[400] text-white">θ {loans.loans.filter((loan) => loan.nftAddress.toLowerCase() == loans.loans[selectedLend].nftAddress.toLowerCase() && loan.amount != 0 && !loan.accepted && !loan.paid && !loan.liquidated).length == 0 ? 0 : formatUnits(loans.loans.filter((loan) => loan.nftAddress.toLowerCase() == loans.loans[selectedLend].nftAddress.toLowerCase() && loan.amount != 0 && !loan.accepted && !loan.paid && !loan.liquidated).sort((a, b) => a.amount - b.amount).at(0).amount, 18)}</span>
              </div>
            </div>
            <div className="w-full flex gap-[20px] justify-between">
              <div className="flex flex-col items-center">
                <span className="text-[10px] font-[400] text-white">INTEREST</span>
                <span className="text-[14px] font-[400] text-[#DBFF00]">θ {Number(formatUnits(loans.loans[selectedLend].amount * (collections.find((collection) => collection.address.toLowerCase() == loans.loans[selectedLend].nftAddress.toLowerCase() && collection.duration == loans.loans[selectedLend].duration).interest) / 1000, 18)).toFixed(2)}</span>
              </div>
              <div className="flex flex-col items-center">
                <span className="text-[10px] font-[400] text-white">DURATION</span>
              ...
              <div className="flex flex-col items-center">
                ...
                <span className="text-[14px] font-[400] text-white">θ {formatUnits(loans.loans[selectedLend].amount, 18)}</span>
              </div>
            </div>
                ...
                {!isLoadingTokens && (tokenIds.length != 0 ?
                  <>
                    ...
                    <div className="w-full flex justify-center">
                      <span className="text-[20px] font-[400] text-white">θ {formatUnits(loans.loans[selectedLend].amount, 18)}</span>
                    </div>
                    <div className="w-full flex justify-center">
